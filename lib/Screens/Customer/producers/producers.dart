...
class _ProducersState extends State<Producers> {
            ...
            children: [
              GestureDetector(
                  onTap: () async{
                    ...
                    //       if(mounted) {
                    //         setState(() {
                    if(_producerTabList[index].expand==false){//imp
                      Producer.setAllExpand(_producerTabList);
                    }
                    // _producerTabList[index].tapcount=producerList[index].tapcount+1;

                    _producerTabList[index].expand=!_producerTabList[index].expand;
                    offset=0;
                    totalProductTabList=[];
                  ...
                  child: Column(
                    ...
                    children: [
                      CachedNetworkImage(
                        imageUrl: _producerTabList[index].producerIconUrl,
                        imageBuilder: (context, imageProvider) {
                          return Container(
                            width: MediaQuery.of(context).size.width,
                            margin: EdgeInsets.only(top: 10.0),
              ...
              if(_producerTabList!=null && _producerTabList[index].expand==true)

                ExpandedSection(productList:__productTabList,totalProductTabList: totalProductTabList,
                    producerList:producerList, expand: _producerTabList[index].expand,index:producerListIndex,
                    cartModel:model,isConnectedToInternet:isconnectedToInternet,pagingController:_pagingTabController,noData:flagDataNotAvailable)
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        key: _scaffoldKey,
        body:
        BlocListener<ProducerProdBloc, ProducerProdState>(listener: (context, state)
        {
          if (state is ProducerListTabLoadFail) {}
        }, child:
        BlocBuilder<ProducerProdBloc, ProducerProdState>(builder: (context, state) {
            ...
            if(__productTabList==null){
              ...
              if (__productTabList.length>0 ) {
                ...
                if (isLastPage) {
                  ...
                  _pagingTabController.appendPage(__productTabList, nextPageKey);
                }
              }else {
...
class ExpandedSection extends StatefulWidget {
  ...
  ExpandedSection({this.expand = false,this.productList,this.totalProductTabList,this.producerList,
    this.index,this.cartModel,this.isConnectedToInternet,this.pagingController,this.noData});
                          ...
                          onPressed: () async {
                            ...
                            if (widget.isConnectedToInternet == true) {
                              ...
                              AddedToCart(model,
                                  // widget.producerList[widget.index]
                                  //     .producerId.toString(),
                                  widget.totalProductTabList[index].producerid.toString(), //updated on 4/01/2022
                                  widget.totalProductTabList[index].productId
                                      .toString(),
                                  widget.totalProductTabList[index].qty
                                      .toString(),
        ...
        if (resp['msg'] == "Successed") {
          ...
          if (model != null) {
            Navigator.push(context, MaterialPageRoute(builder: (context) =>
                ShoppingCart(flagFrom:"0",price: price, cartModel: model,productList: widget.totalProductTabList)));
          }
        } else {
          print('exists');
          Navigator.push(context, MaterialPageRoute(builder: (context) =>
      ...
      Container(
          ...
          Padding(
            ...
            PagedGridView<int, Product>(
                ...
                builderDelegate: PagedChildBuilderDelegate<Product>(
                    itemBuilder: (context, item, index) =>
                        buildListViewItemProd(index, item, widget.cartModel)
                  //     CharacterUpdatedScreen(
                  //   character: item,
                  // ),
                )),
